...
if __name__ == '__main__':
    try:
        ...
        p_d = np.array([5.4, 0.0, 2.0, 90.0])      # Desired Position and Yaw Angle
        count = 0                            # Counter
        r = .2                               # Destination Threshold
        mass = .08                          # Total mass of the blimp in kg
        massThrust = 45000                   # Thrust Gain
        
        #Graph error in x,y,yaw vs time
        ...
        # XY Position
        set_param(cf, 'ctrlMel', 'kp_xy', 0.67) #Default: 0.4
        set_param(cf, 'ctrlMel', 'kd_xy', 0.83) #Default: 0.2
        set_param(cf, 'ctrlMel', 'ki_xy', 0.06) #Default: 0.05
        # Z position
        ...
        # Attitude
        set_param(cf, 'ctrlMel', 'kR_xy', 0.1) #Default: 0.3
        set_param(cf, 'ctrlMel', 'kw_xy', 0.05) #Default: 0.15
        set_param(cf, 'ctrlMel', 'ki_m_xy', 0.1) #Default: 0.0
        # Yaw
        ...
        while(True):
            ...
            log_error_x.append(p_d[0]-p_r[0])
            log_error_y.append(p_d[1]-p_r[1])
            log_error_z.append(p_d[2]-p_r[2])
            log_error_yaw.append(p_d[3]-yaw_r)
            ...
            if count >= 300:
                ...
                # print("--------- Optitrack Quaternion: [x: %.2f, y: %.2f, z: %.2f, w: %.2f]" % (q_r[0],q_r[1],q_r[2],q_r[3]))
        ...
        axes.plot(log_position_x, log_position_y, color="orange", label="Position")
        axes.minorticks_on()
        ...
        ax3[0,1].plot(t1, log_error_x, color="red")
        ax3[0,1].set_title("X error")
        ax3[1,1].plot(t1, log_error_y, color="blue")
        ax3[1,1].set_title("Y error")
        ax3[1,0].plot(t1, log_error_z, color="green")
        ax3[1,0].set_title("Z error")
        ax3[0,0].plot(t1, log_error_yaw, color="orange")
        ax3[0,0].set_title("Yaw error")
        
        ax3[0,0].minorticks_on()
        ax3[0,0].grid(visible=True, which='major', color='#666666', linestyle='-')
        ax3[0,0].grid(visible=True, which='minor', color='#999999', linestyle='-', alpha=0.2)

        ax3[0,1].minorticks_on()
        ax3[0,1].grid(visible=True, which='major', color='#666666', linestyle='-')
        ax3[0,1].grid(visible=True, which='minor', color='#999999', linestyle='-', alpha=0.2)

        ax3[1,0].minorticks_on()
        ax3[1,0].grid(visible=True, which='major', color='#666666', linestyle='-')
        ax3[1,0].grid(visible=True, which='minor', color='#999999', linestyle='-', alpha=0.2)

        ax3[1,1].minorticks_on()
        ax3[1,1].grid(visible=True, which='major', color='#666666', linestyle='-')
        ax3[1,1].grid(visible=True, which='minor', color='#999999', linestyle='-', alpha=0.2)
