...
if __name__ == '__main__':
    try:
        ...
        p_d = np.array([5.4, 0.0, 2.0, 0.0])      # Desired Position and Yaw Angle
        count = 0                            # Counter
        ...
        while(True):
            ...
            if angle > 180:
                angle = -180
            p_d = np.array([2.0*np.cos(np.radians(.02*t)), 2.0*np.sin(np.radians(.02*t)), 1.5, 30*np.sin(0.05*.02*t)]) #30*np.sin(0.05*.02*t)
            now = time.time()
